...
before('Get Token',(done)=>{
    chai.request(server).post('/authenticate').send({username:'mseven',password:'12345'}).end((err,res)=>{
        if(err)
        {
            throw err;
        }
...
describe('/DELETE :director_id',()=>{
    it('Delete Director record by id',(done)=>{
        chai.request(server).del('/api/directors/'+directorId).set('x-access-token',token).end((err,res)=>{
            if(err)
            {
                throw err;
            }
